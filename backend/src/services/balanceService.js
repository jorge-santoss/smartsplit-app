...
const getBalances = async (householdId, userId) => {
  ...
  for (const s of settlements) {
    if (totals[s.from_user_id]) totals[s.from_user_id].net += parseFloat(s.amount);
  ...
  const debtors = balances.filter(b => b.net_balance < -0.005)
    .map(b => ({ ...b, net_balance: Math.abs(b.net_balance) }))
    .sort((a, b) => b.net_balance - a.net_balance);
  const creditors = balances.filter(b => b.net_balance > 0.005)
    .sort((a, b) => b.net_balance - a.net_balance);
